...
{
    char const*const FLASH = "/sys/class/leds/lv5219lg:fled/brightness";
    if (enable){
            int value = 255;
...
static struct msm_frame * cam_frame_get_video()
{
    ...
    if (g_busy_frame_queue.front)
    {
       ...
       LOGV("cam_frame_get_video... out = %lu\n", p->buffer);
    }
    return p;
}
...
static void cam_frame_post_video (struct msm_frame *p)
{
    ...
    LOGV("cam_frame_post_video... out = %lu\n", p->buffer);
...
//    mDimension.ui_thumbnail_width  = THUMBNAIL_HEIGHT;
    ...
    if (initJpegHeap) {
        ...
        // Thumbnails
        int athumbnailBufferSize = 320 * 240 * 3 / 2;
       ...
       const sp<IMemory>& mem __attribute__((unused)))
{
    ...
    if( (mCurrentTarget == TARGET_MSM7630 )  || (mCurrentTarget == TARGET_QSD8250)) {
        ...
        int cnt;
        unsigned int heap_base = (unsigned int)(heap->base());

        for (cnt = 0; cnt < kRecordBufferCount; cnt++) {
            if((unsigned int)recordframes[cnt].buffer == (heap_base + offset)){
           ...
           && height == supportedPreviewSizes[i].height) {
            ...
            if(width == 1280 && height == 720 &&
             ((mCurrentTarget == TARGET_MSM7630) || (mCurrentTarget == TARGET_QSD8250))){
                LOGD("change preview resolution to 854X480 since recording is in 720p");
                mDimension.display_width   = preview_sizes[2].width;
                mDimension.display_height  = preview_sizes[2].height;
                mDimension.postview_width  = 854;
                mDimension.postview_height = 480;
            }else {
...
status_t QualcommCameraHardware::setFlash(const CameraParameters& params)
{
    ...
    LOGV("flash mode value: %s", (str == NULL) ? "NULL" : str);
...
bool QualcommCameraHardware::isValidDimension(int width, int height) {
    bool retVal = FALSE;
    int cwidth = CEILING16(width);
    int cheight = CEILING16(height);
    int tsc = THUMBNAIL_SIZE_COUNT;
